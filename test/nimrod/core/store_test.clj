...
(defn set-and-read-metric-history-with-current-value-only [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" metric-1 {:value "v1" :timestamp 1}]
    ...
    (testing "Metric history values"
      (is (= {:values [metric-1] :size 1 :limit default-limit :median metric-1} (read-history store metric-ns metric-type metric-id #{} nil nil))))))
...
(defn set-and-read-metric-history-with-old-values-too [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" 
    ...
    (testing "Metric history values"
      (is (= {:values [metric-3 metric-2 metric-1] :size 3 :limit default-limit :median metric-2} (read-history store metric-ns metric-type metric-id #{} nil nil))))))
...
(defn set-and-read-metric-history-by-age [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" 
    ...
    (testing "Current metric value"
      (is (= metric-3 (read-metric store metric-ns metric-type metric-id))))
    (testing "Metric history values"
      (is (= {:values [metric-3] :size 1 :limit default-limit :median metric-3} (read-history store metric-ns metric-type metric-id #{} 1000 nil))))))
...
(defn set-and-read-metric-history-by-tags [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" 
    ...
    (testing "Current metric value"
      (is (= metric-3 (read-metric store metric-ns metric-type metric-id))))
    (testing "Metric history values"
      (is (= {:values [metric-1] :size 1 :limit default-limit :median metric-1} (read-history store metric-ns metric-type metric-id #{"t"} nil nil))))))
...
(defn set-and-read-metric-history-with-limit [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" 
    ...
    (testing "Metric history values with limit"
      (is (= {:values [metric-3 metric-2] :size 2 :limit 2 :median metric-3} (read-history store metric-ns metric-type metric-id #{} nil 2))))))
...
(defn set-and-remove-metric-history-by-id-and-age [store]
  (let [metric-ns "1" metric-type "alert" metric-id "1" 
    ...
    (testing "Current metric value"
      (is (= metric-3 (read-metric store metric-ns metric-type metric-id))))
    (testing "Metric history values"
      (is (= {:values [metric-3] :size 1 :limit default-limit :median metric-3} (read-history store metric-ns metric-type metric-id #{} nil nil))))))
...
(defn set-and-remove-multiple-metrics-history-by-age [store]
  (let [metric-ns "1" metric-type "alert" metric-id-1 "1" metric-id-2 "2" metric-id-3 "3" 
    ...
    (testing "Metric history values"
      (is (= {:values [metric-3] :size 1 :limit default-limit :median metric-3} (read-history store metric-ns metric-type metric-id-3 #{} nil nil))))))
...
(defn set-and-merge-metric-history [store]
  (let [metric-ns "1" metric-type "alert" 
    ...
    (testing "Merged metric history values"
      (is (= {:values [metric-3 metric-2] :size 2 :limit 2 :median metric-3} (merge-history store metric-ns metric-type #{"t"} nil 2))))))
